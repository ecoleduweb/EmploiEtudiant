...
class JobOffer(db.Model):
    ...
    def __repr__(self):
        return f"JobOffer(id={self.id}, title='{self.title}', address='{self.address}', description='{self.description}', dateDisplayJobOffer='{self.dateDisplayJobOffer}' , dateEntryOffice='{self.dateEntryOffice}', deadlineApply='{self.deadlineApply}', email='{self.email}', hoursPerWeek={self.hoursPerWeek}, compliantEmployer={self.compliantEmployer}, internship={self.internship}, offerStatus={self.offerStatus}, offerLink='{self.offerLink}', salary='{self.salary}', urgent={self.urgent}, active={self.active}, employerId={self.employerId}, scheduleId={self.scheduleId})"
