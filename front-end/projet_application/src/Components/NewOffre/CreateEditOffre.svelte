<script lang="ts">
    ...
    const fetchEnterprise = async () => {
        ...
        if (isJobOfferEdit === true) {
            ...
            if (employer)
                response = await GET<any>(
                    "/enterprise/getEnterpriseByEmployer?id=" + employer.id,
                )
        }
        if (response !== undefined) {
            entreprise = response
            const city = villesOption.find(
                (ville) => ville.value === entreprise.cityId,
            )
            if (city) {
                villeSelected = [city]
            }
    ...
    <form on:submit|preventDefault={handleSubmit} class="form-offre">
        <div class="content-form">
                ...
                {:else}
                    ...
                    <div class="form-group-horizontal">
                        <MultiSelect
                            ...
                            on:add={(event) =>
                                getEnterprise(event.detail.option.value)}
                        />
                        <Button
                            submit={false}
                            text="Ajouter"
            ...
            {#if offre.approbationMessage}
                <h3 style="color: red;">
                    Raison du refus: {offre.approbationMessage}
                </h3>
            {/if}
            {#if isJobOfferEdit === true}
