<script lang="ts">
    ...
    const addStudy = async (newName: string) => {
        try {
            const response = await POST<any, any>(`/studyProgram/addStudyProgram`, 
            {
                name: newName
            })
...
<div class="main-div">
    <div class="container">
        <div>
            <h5 class="infoTitle">Veuillez choisir un nouveau nom pour le nouveau programme</h5>
            <input type="text"
        ...
        <div class="button">
            <Button text="Modifier" onClick={() => addStudy(nameChosen)} />
